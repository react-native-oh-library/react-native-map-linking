...
function openDialog(urls) {
  if (_reactNative.Platform.OS === 'ios') {
    urls = urls.ios;
    return Promise.all(urls.map(element => _reactNative.Linking.canOpenURL(element[1]))).then(results => {
      return urls.filter((element, index) => results[index]);
    }).then(choices => {
      // 系统内没有任何地图, 推荐下载一个
      if (choices.length < 1) {
        return _reactNative.ActionSheetIOS.showActionSheetWithOptions({
          options: ['下载高德地图', '下载百度地图', '取消'],
          cancelButtonIndex: 2,
          title: '选择地图'
        }, buttonIndex => {
          if (buttonIndex === 0) {
            _reactNative.Linking.openURL('https://itunes.apple.com/cn/app/gao-tu-zhuan-ye-shou-ji-tu/id461703208?mt=8');
          } else if (buttonIndex === 1) {
            _reactNative.Linking.openURL('https://itunes.apple.com/cn/app/bai-du-tu-shou-ji-tu-lu-xian/id452186370?mt=8');
          }
        });
      }
      return _reactNative.ActionSheetIOS.showActionSheetWithOptions({
        ...
        if (buttonIndex < choices.length) {
          _reactNative.Linking.openURL(choices[buttonIndex][1]);
        }
      });
    });
    ...
    return Promise.all(urls.map(element => _reactNative.Linking.canOpenURL(element[1]))).then(results => {
      return urls.filter((element, index) => results[index]).map(url => ({
        text: url[0],
        onPress: () => {
          _reactNative.Linking.openURL(url[1]);
        }
      }));
    }).then(choices => {
      // 系统内没有任何地图, 推荐下载一个
      if (choices.length < 1) {
        return _reactNative.Alert.alert('选择地图', '您还没有安装地图软件。', [{
          text: '下载高德地图',
          onPress: () => _reactNative.Linking.openURL('http://mobile.amap.com')
        }, {
          text: '下载百度地图',
          onPress: () => _reactNative.Linking.openURL('http://map.baidu.com')
        }, {
          text: '取消'
        }]);
      }
      return _reactNative.Alert.alert('选择地图', '请选择一个地图打开', [...choices, {
        text: '取消'
      }]);
    });
    ...
    return Promise.all(urls.map(element => _reactNative.Linking.canOpenURL(element[1]))).then(results => {
      ...
      if (choices.length < 1) {
        return _reactNative.Alert.alert('选择地图', '您还没有安装地图软件。', [{
          text: '下载高德地图',
          onPress: () => _reactNative.Linking.openURL('store://appgallery.huawei.com/app/detail?id=C1268244389566675520')
        }, {
          text: '下载百度地图',
          onPress: () => _reactNative.Linking.openURL('store://appgallery.huawei.com/app/detail?id=C1268244389566675520')
        },
...
var _default = exports.default = {
  ...
  markLocation(location, title, content) {
    return openDialog({
      ...
      harmony: [['使用高德地图打开', `iosamap://viewMap?sourceApplication=${this.options.appName}&poiname=${title}&lat=${location.lat}&lon=${location.lng}&dev=${location.type === 'gcj02' ? '0' : '1'}`], ['使用百度地图打开', `baidumap://map/marker?location=${location.lat},${location.lng}&coord_type=${location.type === 'gcj02' ? 'gcj02' : 'wgs84'}&title=${title}&content=${content}&src=${this.options.appName}`], ['使用花瓣地图打开', `maps://locationInfo?linkSource=${this.options.appName}&destinationLatitude=${location.lat}&destinationLongitude=${location.lng}&destinationName=${title}`]]
    });
  },
  /**
   * 规划线路
