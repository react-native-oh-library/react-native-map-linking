...
function openDialog(urls) {
  if (Platform.OS === 'ios') {
    urls = urls.ios;
    return Promise.all(urls.map(element => Linking.canOpenURL(element[1]))).then(results => {
    ...
    return Promise.all(urls.map(element => Linking.canOpenURL(element[1]))).then(results => {
      ...
      return Alert.alert('选择地图', '请选择一个地图打开', [...choices, {
        text: '取消'
      }]);
    });
    ...
    return Promise.all(urls.map(element => Linking.canOpenURL(element[1]))).then(results => {
      ...
      if (choices.length < 1) {
        return Alert.alert('选择地图', '您还没有安装地图软件。', [{
          text: '下载高德地图',
          onPress: () => Linking.openURL('store://appgallery.huawei.com/app/detail?id=C1268244389566675520')
        }, {
          text: '下载百度地图',
          onPress: () => Linking.openURL('store://appgallery.huawei.com/app/detail?id=C1268244389566675520')
        },
